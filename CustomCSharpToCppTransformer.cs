...
namespace CSharpToCppTranslator
{
    public class CustomCSharpToCppTransformer : CSharpToCppTransformer
    {
        public static readonly IList<ISubstitutionRule> Rules = new List<SubstitutionRule>
        {
            ...
            (new Regex(@"\r?\n[\t ]+[^\r\n]*GetZero(.|\s)+Increment\(One\)(.|\s)+?}"), "", new Regex(@"GenericCollectionMethodsBase\.cs"), 0),
            // Just delete it in SizedBinaryTreeMethodsBase.cs
            (new Regex(@"\r?\n[\t ]+[^\r\n]*FixSizes(.|\s)+};"), "    };", new Regex(@"SizedBinaryTreeMethodsBase\.cs"), 0),
            ...
            (new Regex(@"([a-zA-Z0-9]+)\(std::int32_t capacity\) { \(_elements, _allocated\) = \(new TreeElement\[capacity\], 1\); }"), "$1() { _allocated = 1; }", new Regex(@"Size[a-zA-Z]+Tree\.cs"), 0),
